...
async function create(data) {
  ...
  const result = await db.query(
    `
      INSERT INTO download_strategy_rollout_circuit_breakers (
        rollout_id,
        state,
        failure_threshold,
        success_threshold,
...
async function findById(id) {
  ...
  const result = await db.query(
    `
      SELECT *
      FROM download_strategy_rollout_circuit_breakers
      WHERE id = $1
      LIMIT 1
    `,
    [id]
...
async function findByRolloutId(rolloutId) {
  const db = getClient();
...
async function update(id, updates) {
  ...
  const result = await db.query(
    `
      UPDATE download_strategy_rollout_circuit_breakers
      SET ${setClause},
          updated_at = NOW()
      WHERE id = $1
      RETURNING *
