...
async function create(data) {
  ...
  const result = await db.query(
    `
      INSERT INTO download_strategy_rollout_strategy_metrics (
        rollout_id,
        strategy_id,
        period_start,
        period_end,
...
async function findById(id) {
  ...
  const result = await db.query(
    `
      SELECT *
      FROM download_strategy_rollout_strategy_metrics
      WHERE id = $1
      LIMIT 1
    `,
    [id]
...
async function findByStrategyAndPeriod(
  rolloutId,
  strategyId,
  periodStart
) {
  ...
  const result = await db.query(
    `
      ...
      LIMIT 1
    `,
    [rolloutId, strategyId, periodStart]
  );
...
async function update(
  ...
  if (entries.length === 0) {
    return findByStrategyAndPeriod(
      rolloutId,
      strategyId,
      periodStart
    );
  ...
  const result = await db.query(
    `
      UPDATE download_strategy_rollout_strategy_metrics
      SET ${setClause},
          updated_at = NOW()
      WHERE rollout_id = $1
        AND strategy_id = $2
...
async function deleteByRolloutId(rolloutId) {
  ...
  const result = await db.query(
    `
      DELETE FROM download_strategy_rollout_strategy_metrics
      WHERE rollout_id = $1
    `,
    [rolloutId]
  );
...
async function deleteByStrategyId(strategyId) {
  const db = getClient();
