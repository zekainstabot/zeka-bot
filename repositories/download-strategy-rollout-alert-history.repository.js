...
async function create(data) {
  ...
  const result = await db.query(
    `
      INSERT INTO download_strategy_rollout_alert_history (
        rollout_id,
        alert_id,
        alert_type,
        previous_status,
...
async function findById(id) {
  ...
  const result = await db.query(
    `
      SELECT *
      FROM download_strategy_rollout_alert_history
      WHERE id = $1
      LIMIT 1
    `,
    [id]
...
async function findByAlertId(alertId, limit = 500) {
  const db = getClient();
...
async function findByAlertType(alertType, limit = 500) {
  const db = getClient();
...
async function deleteByRolloutId(rolloutId) {
  ...
  const result = await db.query(
    `
      DELETE FROM download_strategy_rollout_alert_history
      WHERE rollout_id = $1
    `,
    [rolloutId]
  );
